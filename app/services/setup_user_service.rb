...
class SetupUserService
  ...
  def initialize(provider_name:, email:, application_name:, role:, municipality_id:)
    ...
    user = User.create(email: email, password: user_password, password_confirmation: user_password, role: role, municipality: self)
    ...
    OnePasswordService.setup(municipality_id: municipality_id, password: user_password, username: email) if user.id.present? && Rails.env.production?
  end
