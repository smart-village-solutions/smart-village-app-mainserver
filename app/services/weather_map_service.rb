...
class WeatherMapService
  ...
  def initialize
    return if Settings.config[:openweathermap].blank?

    lat = Settings.config[:openweathermap][:lat]
    lon = Settings.config[:openweathermap][:lon]
    api_key = Settings.config[:openweathermap][:api_key]
