...
class EventRecordService
  ...
  def self.update_sort_dates # rubocop:disable Metrics/AbcSize
    p "EventRecordService started at #{Time.zone.now}"
    Municipality.all.each do |municipality|
      MunicipalityService.municipality_id = municipality.id
      p "EventRecordService for municipality #{municipality.id}"
