...
module Mutations
  class CreatePointOfInterest < BaseMutation
    ...
    argument :addresses, [Types::AddressInput], required: false, as: :addresses_attributes, prepare: ->(addresses, _ctx) { addresses.map(&:to_h) }
    argument :contact, Types::ContactInput, required: false, as: :contact_attributes, prepare: ->(contact, _ctx) { contact.to_h }
    argument :prices, [Types::PriceInput], required: false, as: :prices_attributes, prepare: ->(prices, _ctx) { prices.map(&:to_h) }
    argument :opening_hours, [Types::OpeningHourInput], required: false, as: :opening_hours_attributes, prepare: ->(opening_hours, _ctx) { opening_hours.map(&:to_h) }
    argument :data_provider, Types::DataProviderInput, required: false, as: :data_provider_attributes, prepare: ->(data_provider, _ctx) { data_provider.to_h }
    argument :operating_company, Types::OperatingCompanyInput, required: false, as: :operating_company_attributes, prepare: ->(operating_company, _ctx) { operating_company.to_h }
    argument :web_urls, [Types::WebUrlInput], required: false, as: :web_urls_attributes, prepare: ->(web_urls, _ctx) { web_urls.map(&:to_h) }
