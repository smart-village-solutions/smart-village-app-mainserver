...
RSpec.describe ExternalServices::EventRecords::EventSyncService do
  ...
  let(:data_provider) { instance_double(DataProvider, external_service: external_service, external_service_credential: credential) }
  let(:external_service) { instance_double(ExternalService, external_service_credentials: [credential], base_uri: "https://example.com", resource_config: resource_config, id: 999) }
  let(:credential) { instance_double(ExternalServiceCredential, additional_params: { organization_id: 123 }) }
  ...
  let(:external_service_preparer) { instance_double(ExternalServices::EventRecords::OvedaPreparer) }
  ...
  before do
    ...
    allow(ExternalServices::EventRecords::OvedaPreparer).to receive(:new).with(event).and_return(external_service_preparer)
